...
### Advanced Randomize
The advanced randomize algorithm gets steered by a simple heuristic. It chooses a random departure station whereafter it chooses the connection which has been used the least in a particular solution. For example if a solution consists of 19 trajectories, it keeps track of how many times each connection has been used within these 19 trajectories. If a 20th trajectory is made it will then depart from a random station and choose from it's possible connections, the connection which has been used the least by the other 19 trajectories. 
